    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-amber-500 selection:text-slate-950">
      ...
      <section className="relative py-24 px-4 sm:px-6 lg:px-8 border-b border-slate-900 bg-gradient-to-b from-slate-900 to-slate-950 text-center">
        ...
        <div className="max-w-4xl mx-auto space-y-4 relative z-10">
          ...
          <p className="text-slate-400 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed">
            Discover our operational guidelines, strict on-site safety policies, and the dedicated personnel managing local masonry restorations.
          </p>
        </div>
      </section>

      {/* SECTION 2: THE FOUNDING BLUEPRINT STORY */}
      <section className="py-20 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          
          <div className="lg:col-span-6 space-y-6">
            <span className="text-xs font-bold text-amber-500 uppercase tracking-widest font-mono">Our Heritage</span>
            <h2 className="text-3xl sm:text-4xl font-black text-white uppercase tracking-tight">
              Engineering Local Resilience, <br />
              <span className="text-amber-500">Borough by Borough</span>
            </h2>
            <p className="text-sm sm:text-base text-slate-400 leading-relaxed">
              Founded on the belief that masonry restoration and concrete flatwork require zero compromise, we have built a reputation across local sectors for robust execution. We do not skip structural steps, cut back on curing chemicals, or hide behind stock assets.
            </p>
            <p className="text-sm text-slate-400 leading-relaxed">
              Whether deploying high-rigging dust barriers for extensive brick pointing jobs in Queens or calculating complex pitch layouts for a commercial driveway pour, our crews deliver uncompromised structural performance.
            </p>
            
            <div className="border-l-2 border-amber-500 pl-4 py-1 italic text-slate-300 text-sm bg-amber-500/[0.02]">
              "We measure our corporate success by the years our joints remain weather-sealed and the speed at which city violations clear your title."
            </div>
          </div>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-900 bg-gradient-to-b from-slate-950 via-slate-900/10 to-slate-950">
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
          <span className="text-xs font-bold text-amber-500 uppercase tracking-widest font-mono">Core Values</span>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight uppercase">Our Operational Standards</h2>
          <p className="text-slate-400 text-sm">
            Every crew member on an active lot follows these foundational compliance markers.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {companyPillars.map((pillar, idx) => (
            <div key={idx} className="bg-slate-900/20 border border-slate-900 p-6 rounded-2xl space-y-4 hover:border-slate-800 transition-colors group">
              ...
              <h3 className="text-lg font-bold text-white tracking-tight group-hover:text-amber-400 transition-colors">{pillar.title}</h3>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto border-t border-slate-900">
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-3">
          <span className="text-xs font-bold text-amber-500 uppercase tracking-widest font-mono">Field Commanders</span>
          <h2 className="text-3xl sm:text-4xl font-extrabold text-white tracking-tight uppercase">Command & Coordination</h2>
          <p className="text-slate-400 text-sm">
            Direct endpoints to access our project supervisors, routing specialists, and administrators instantly.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
          {teamMembers.map((member, index) => (
            <div key={index} className="bg-gradient-to-b from-slate-900/50 to-slate-900/10 border border-slate-900 p-6 sm:p-8 rounded-3xl space-y-4 flex flex-col justify-between hover:border-slate-800 transition-colors">
              <div className="space-y-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-amber-400 font-mono tracking-wider uppercase">{member.role}</p>
                  </div>
                </div>
              ...
              <div className="pt-4">
                ...
                >
                  <Phone className="w-3.5 h-3.5 text-amber-500" /> Connect: {member.phone}
                </a>
              </div>
            </div>
          ))}
      ...
      <section className="py-16 px-4 max-w-5xl mx-auto border-t border-slate-900">
        <div className="bg-slate-900/10 border border-slate-900 rounded-3xl p-6 sm:p-8 space-y-6">
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-xl font-bold text-white tracking-tight uppercase">Vetted Corporate Protection Framework</h3>
          </div>
          <p className="text-xs sm:text-sm text-slate-400 leading-relaxed">
            Property managers and residential boards across New York demand concrete proof of coverage before allowing equipment on site. We provide comprehensive safety files directly to your board or digital screening vendor upon contract finalization.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 font-mono text-xs text-slate-300">
            <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-amber-500" /> General Liability Certified</div>
            <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-amber-500" /> Workers Comp Enabled</div>
            <div className="flex items-center gap-2"><CheckCircle2 className="w-4 h-4 text-amber-500" /> Scaffold Rigging Compliant</div>
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-4 text-center bg-gradient-to-t from-slate-900 to-transparent border-t border-slate-900/40">
        <div className="max-w-2xl mx-auto space-y-6">
          ...
          <h2 className="text-3xl font-black tracking-tight text-white uppercase">Deploy Our Professional Crew</h2>
          <p className="text-slate-400 text-sm max-w-md mx-auto leading-relaxed">
            Put specialized masonry machinery and seasoned contractors to work on your structural framework today.
          </p>
          <div className="pt-2">
            <a 
              href="tel:9292789239" 
            ...
            >
              <Phone className="w-4 h-4 fill-current" /> Call Ricky For Free Estimate
            </a>
          </div>
        </div>
      </section>
