  ...
  const portfolioItems = [
    {
      ...
      location: "Queens, NY",
      scope:
        "Structural masonry steps featuring premium polished stone treads and custom brick veneer risers.",
    },
    // {
    ...
    // },
    {
      ...
      scope:
        "Freshly poured continuous concrete flatwork with precision tool-cut expansion joints for seasonal shifting.",
    },
    // {
    ...
    // },
    {
      ...
      scope:
        "Finished structural boundary setup with freshly poured smooth concrete walkway and capped brick pillars.",
    },
    {
    ...
    },
    {
      ...
      location: "Long Island, NY",
      scope:
        "Seamless architectural integration of custom stone-capped masonry pillars and graded concrete paving.",
    },
    {
    ...
    },
    {
      ...
      location: "Queens, NY",
      scope:
        "Extensive architectural facade rebuild featuring integrated stone steps and weather-sealed deep pointing.",
    },
    {
      ...
      location: "Brooklyn, NY",
      scope:
        "Active mortar application and structural cinder block reinforcement within a load-bearing envelope opening.",
    },
    {
      ...
      scope:
        "Targeted mortar extraction and precision brick replacement to restore full facade integrity.",
    },
    {
      ...
      location: "Queens, NY",
      scope:
        "Installation and structural restoration of decorative architectural stone balusters and railing components.",
    },
    {
      ...
      <section className="relative py-24 px-4 sm:px-6 lg:px-8 border-b border-slate-900 bg-gradient-to-b from-slate-900 to-slate-950 text-center">
        ...
        <div className="max-w-4xl mx-auto space-y-4 relative z-10">
          ...
          <p className="text-slate-400 text-base sm:text-lg max-w-2xl mx-auto leading-relaxed">
            No stock files, no digital renders. Browse unedited verification
            captures straight from our active structural sites across New York.
          </p>
        </div>
      </section>
      ...
      <section className="py-8 px-4 sm:px-6 lg:px-8 w-full sticky top-0 z-40 bg-slate-950/90 backdrop-blur-md border-b border-slate-900/60 shadow-lg">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-center gap-2 sm:gap-3">
          {categories.map((category) => (
            ...
            >
              {category === "All" && (
                <Grid className="w-3.5 h-3.5 inline mr-1.5 -mt-0.5" />
              )}
              {category === "Parapet Walls & Lintels" && (
                <Hammer className="w-3.5 h-3.5 inline mr-1.5 -mt-0.5" />
              )}
              {category === "Brick Grinding & Pointing" && (
                <Layers className="w-3.5 h-3.5 inline mr-1.5 -mt-0.5" />
              )}
              {category === "Concrete Flatwork" && (
                <Construction className="w-3.5 h-3.5 inline mr-1.5 -mt-0.5" />
              )}
              {category === "Waterproofing & Restoration" && (
                <ShieldCheck className="w-3.5 h-3.5 inline mr-1.5 -mt-0.5" />
              )}
              {category === "Roofing" && (
                <FolderGit2 className="w-3.5 h-3.5 inline mr-1.5 -mt-0.5" />
              )}
              {category === "Painting & Restoration" && (
                <Layers className="w-3.5 h-3.5 inline mr-1.5 -mt-0.5" />
              )}
        ...
        {filteredItems.length === 0 && (
          <div className="py-24 text-center max-w-md mx-auto space-y-2">
            <p className="text-base font-bold text-white">
              No explicit items match this category type
            </p>
            <p className="text-xs text-slate-500">
              Try modifying your filtering selection parameters using the
      ...
      <section className="py-20 px-4 text-center bg-gradient-to-t from-slate-900 to-transparent border-t border-slate-900/40">
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-slate-800 text-xs font-mono text-slate-400 bg-slate-950">
            ...
            Free Commercial Consultations Offered Across NYC
          </div>
          <h2 className="text-3xl font-black tracking-tight text-white uppercase">
            Request a Field Estimate
          </h2>
          <p className="text-slate-400 text-sm max-w-md mx-auto leading-relaxed">
            Ready to add your project blueprint to our gallery? Call our team
            layout directly to schedule raw surface inspections.
          </p>
          <div className="pt-2">
            <a
              href="tel:9292789239"
            ...
            >
              <Phone className="w-4 h-4 fill-current" /> Contact Ricky:
              929-278-9239
            </a>
          </div>
        </div>
